...
async function main() {
  ...
  if (!program.programId.equals(PROGRAM_ID))
    throw new Error("IDL program id is not the deployed one");
  ...
  const lpUsdcAta = getAssociatedTokenAddressSync(
    USDC_DEVNET,
    LP.publicKey,
    false,
    TOKEN_PROGRAM_ID,
  ...
  const [poolShareAccount] = PublicKey.findProgramAddressSync(
    [Buffer.from("pool_shares")],
    program.programId,
  );
  ...
  console.log(
    "  sig",
    await program.methods.setPauseFlags(new anchor.BN(0)).accounts({ admin }).rpc(),
  );
  ...
  if (lpUsdc < DEPOSIT) {
    const fund = new Transaction()
      .add(
        SystemProgram.transfer({ fromPubkey: admin, toPubkey: LP.publicKey, lamports: 30_000_000 }),
      )
      .add(
        createAssociatedTokenAccountIdempotentInstruction(
          admin,
  ...
  const sharesBefore = (await getAccount(connection, shareAta, "confirmed", TOKEN_PROGRAM_ID))
    .amount;
  ...
  const vaultAfter = (await getAccount(connection, quoteVault, "confirmed", TOKEN_PROGRAM_ID))
    .amount;
